...
#region Consts
        ...
        /// <summary>
        /// GB2312-80 标准规范中第一个汉字（即“啊”）的机内码。
        /// </summary>
        private const int firstChCode = -20319;

        /// <summary>
        ///  GB2312-80 标准规范中最后一个汉字（即“齄”）的机内码。
        /// </summary>
...
#region Methods
        ...
        /// <returns></returns>        
        public static string GetFirstChars(char chr)
        {
            string rs = GetFullChars(chr);
        ...
        public static string GetFirstChars(string str)
        {
            ...
            for (int i = 0, len = chrs.Length; i < len; i++)
            {
                builder.Append(GetFirstChars(chrs[i]));
            }
        ...
        /// <param name="chr"></param>
        /// <returns></returns>
        public static string GetFullChars(char chr)
        {
            // 拉丁字符            
            if (chr <= '\x00FF') return chr.ToString();

            // 标点符号、分隔符            
            if (char.IsPunctuation(chr) || char.IsSeparator(chr)) return chr.ToString();

            // 非中文字符            
            if (chr < '\x4E00' || chr > '\x9FA5') return chr.ToString();
            ...
            {
                throw new NotSupportedException("Not support GB2312 / GBK encoding.", ex);
            }

            byte[] arr = encoding.GetBytes(chr.ToString());
            int n = (Int16)arr[0] * 256 + (Int16)arr[1] - 65536;

            // 单字符（英文或半角字符） 
            if (n > 0 && n < 160) return chr.ToString();

            // 判断是否超过 GB2312-80 标准中的汉字范围
            if (n > lastChCode || n < firstChCode)
            {
                return chr.ToString(); ;
            }

            // 如果是在一级汉字中
            else if (n <= lastOfOneLevelChCode)
            {
                // 将一级汉字分为12块，每块33个汉字.
                for (int aPos = 11; aPos >= 0; aPos--)
                {
                    ...
                    if (n >= pyValue[aboutPos])
                    {
                        // 遍历块中的每个音节机内码,从最后的音节机内码开始扫描，如果音节内码小于机内码，则取此音节
            ...
            {
                int pos = Array.IndexOf(otherChinese, chr.ToString());
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static string GetFullChars(string str)
        {
            ...
            for (int i = 0, len = chrs.Length; i < len; i++)
            {
                builder.Append(GetFullChars(chrs[i]));
            }
